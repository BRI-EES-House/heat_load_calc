...
def get_OTset(Vel: float, RH: float, Clo: float, PMV_set: float, h_c_i_n, t_cl_i_n) -> float:
    """ 作用温度の計算
...
def calc_OTset(now_air_conditioning_mode: int, isRadiantHeater: bool, RH: float, PMV_set: float, h_c_i_n, t_cl_i_n) -> float:
    """
    ...
    if now_air_conditioning_mode != 0 and not isRadiantHeater:
        ...
        OTset = get_OTset(Vel, RH, Clo, PMV_set, h_c_i_n, t_cl_i_n)
    # 放射式空調時
