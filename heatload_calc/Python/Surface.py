...
class Surface:
    ...
    def __init__(self, surfaces, solar_position, I_DN_n, I_sky_n, RN_n, To_n, AnnualTave):
        ...
        # 隣室温度差係数
        self.a_i_k = np.zeros(self.Nsurf)
        ...
        self.next_room_type = np.zeros(self.Nsurf, dtype=np.object)
        for k, d in enumerate(surfaces):
            if self.boundary_type[k] in ["internal"]:
                self.next_room_type[k] = d['next_room_type']

        # 外皮の場合
        self.direction = np.zeros(self.Nsurf, dtype=np.object)
        self.Type = np.zeros(self.Nsurf, dtype=np.object)
        self.nextroomname = np.zeros(self.Nsurf, dtype=np.object)
        self.RhoG_l = np.zeros(self.Nsurf)
        self.Wa = np.zeros(self.Nsurf)
        self.Wb = np.zeros(self.Nsurf)
        self.Wz = np.zeros(self.Nsurf)
        self.WW = np.zeros(self.Nsurf)
        self.Ws = np.zeros(self.Nsurf)
        self.PhiS_i_k = np.zeros(self.Nsurf)
        self.PhiG_i_k = np.zeros(self.Nsurf)
        ...
        for k, d in enumerate(surfaces):
            if self.boundary_type[k] in ["external_general_part", "external_transparent_part", "external_opaque_part"]:
                ...
                # 方位角、傾斜面方位角 [rad]
                self.Wa[k], self.Wb[k] = a19.get_slope_angle(self.direction[k])

                # 傾斜面に関する変数であり、式(73)
                self.Wz[k], self.WW[k], self.Ws[k] = a19.get_Wz_Ww_Ws(self.Wa[k], self.Wb[k])

                # 傾斜面の天空に対する形態係数の計算 式(120)
                self.PhiS_i_k[k] = a19.get_Phi_S_i_k(self.Wz[k])
            ...
            # 内壁の場合
                ...
                self.nextroomname[k] = self.next_room_type[k] # 隣室名称
        ...
        self.sunbrk = [None] * self.Nsurf
        for k, d in enumerate(surfaces):
            if self.is_sun_striked_outside[k]:
                self.sunbrk[k] = d['solar_shading_part']         # ひさし

        # 壁体の種別
        is_general_part = [(self.boundary_type[k] in ["external_general_part", "internal", "ground"]) for k in range(self.Nsurf)]
        is_transparent_part = [self.boundary_type[k] in ["external_transparent_part"] for k in range(self.Nsurf)]
        is_opaque_part = [self.boundary_type[k] in ["external_opaque_part"] for k in range(self.Nsurf)]
        is_ground = [self.boundary_type[k] == "ground" for k in range(self.Nsurf)]     # 壁体に土壌が含まれる場合True
        ...
        }[self.boundary_type[k]] for k in range(self.Nsurf)]

        # 読み出し
        datalist = [s[k] for (s,k) in zip(surfaces, part_key_name)]

        # 透過率・拡散日射の入射角特性
        self.tau = np.zeros(self.Nsurf)
        self.incident_angle_characteristics = np.zeros(self.Nsurf, dtype=np.object)
        self.Cd = np.zeros(self.Nsurf)
        for k, data in enumerate(datalist):
            if is_transparent_part[k]:
                self.tau[k] = float(data['eta_value'])
                self.incident_angle_characteristics[k] = data['incident_angle_characteristics']
                self.Cd[k] = a25.get_Cd(self.incident_angle_characteristics[k])

        # 室外側日射吸収率・室外側放射率
        self.as_i_k = np.zeros(self.Nsurf)
        self.eps_i_k = np.zeros(self.Nsurf)
        for k, data in enumerate(datalist):
            if self.is_sun_striked_outside[k]:
        ...
        # 室外側熱伝達率
        self.ho_i_k_n = np.zeros(self.Nsurf)
        for k, data in enumerate(datalist):
        ...
        self.hi_i_k_n = np.zeros(self.Nsurf)
        Ri_i_k_n = np.zeros(self.Nsurf)
        for k, data in enumerate(datalist):
        ...
        self.U = np.zeros(self.Nsurf)
        self.Uso = np.zeros(self.Nsurf)
        for k, data in enumerate(datalist):
        ...
        self.RFT0, self.RFA0, self.RFT1, self.RFA1, self.Row, self.Nroot = \
            np.zeros(self.Nsurf), np.zeros(self.Nsurf), np.zeros((self.Nsurf,12)), np.zeros((self.Nsurf,12)),\
            np.zeros((self.Nsurf,12)), np.zeros(self.Nsurf, dtype=np.int64)
        for k, data in enumerate(datalist):
            if is_general_part[k] == False:
        ...
        # 外表面に日射が当たる場合
        self.cos_Theta_i_k_n = np.zeros((self.Nsurf, 24*365*4))
        self.Iw_i_k_n = np.zeros((self.Nsurf, 24*365*4))
        self.I_D_i_k_n = np.zeros((self.Nsurf, 24*365*4))
        self.I_S_i_k_n = np.zeros((self.Nsurf, 24*365*4))
        self.I_R_i_k_n = np.zeros((self.Nsurf, 24*365*4))
        for k, data in enumerate(datalist):
        ...
        # 外表面に日射が当たる場合
        self.Teolist = np.zeros((self.Nsurf, 24*365*4))
        for k, data in enumerate(datalist):
            if self.is_sun_striked_outside[k]:
        ...
        # 外表面に日射が当たる場合
        self.F_SDW_i_k = np.zeros((self.Nsurf, 24*365*4))
        self.QGT_i_k_n = np.zeros((self.Nsurf, 24*365*4))
        for k, data in enumerate(datalist):
            if self.is_sun_striked_outside[k]:
                ...
                if self.boundary_type[k] == "external_transparent_part":
                    ...
                    if self.sunbrk[k]['existance']:
                        if self.sunbrk[k]['input_method'] == 'simple':
                            self.F_SDW_i_k[k] = a8.calc_F_SDW_i_k(
                        ...
                        self.F_SDW_i_k[k,:] = 0.0

                    # 透過日射熱取得
                    self.QGT_i_k_n[k] = a11.calc_QGT_i_k_n(
                        cos_Theta_i_k_n=self.cos_Theta_i_k_n[k],
                        incident_angle_characteristics=self.incident_angle_characteristics[k],
                        I_D_i_k_n=self.I_D_i_k_n[k],
                        F_SDW_i_k=self.F_SDW_i_k[k],
