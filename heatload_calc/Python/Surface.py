...
class Surface:
    ...
    def __init__(self, surfaces):
        ...
        self.sunbrk = read_to_array('solar_shading_part')
        ...
        # 透過率・拡散日射の入射角特性
        self.tau_i_k = read_from_datalist_float('eta_value', 0.0)
        self.IAC_i_k = read_from_datalist('incident_angle_characteristics', None)

        # 室外側日射吸収率
        self.as_i_k = read_from_datalist_float('outside_solar_absorption', 0.0)

        # 室外側放射率[-]
        self.eps_i_k = read_from_datalist_float('outside_emissivity', 0.0)

        # 室外側熱伝達率
        self.Ro_i_k_n = read_from_datalist_float('outside_heat_transfer_resistance', 0.0)

        # 室内側熱伝達抵抗・室内側表面総合熱伝達率
        self.Ri_i_k_n = read_from_datalist('inside_heat_transfer_resistance')  # 室内側熱伝達抵抗

        # 開口部熱貫流率[W/m2K]
        self.U = read_from_datalist_float('u_value', 0.0)

        # 応答係数の準備
        self.R_i_k_p = [None] * len(datalist)  # 熱抵抗
        self.C_i_k_p = [None] * len(datalist)  # 熱容量
        for k, data in enumerate(datalist):
            if self.boundary_type[k] in ["external_general_part", "internal", "ground"]:
                ...
                self.R_i_k_p[k], self.C_i_k_p[k] = a24.get_layers(data, is_ground)
        ...
        self.hi_i_k_n = np.zeros((self.N_surf_i, 24 * 365 * 4))
        self.ho_i_k_n = np.zeros((self.N_surf_i, 24 * 365 * 4))
        ...
        self.RFT0, self.RFA0, self.RFT1, self.RFA1, self.Row, self.Nroot = \
            np.zeros(self.N_surf_i), np.zeros(self.N_surf_i), np.zeros((self.N_surf_i, 12)), np.zeros(
                (self.N_surf_i, 12)), \
            np.zeros((self.N_surf_i, 12)), np.zeros(self.N_surf_i, dtype=np.int64)

    def init(self, solar_position, I_DN_n, I_sky_n, RN_n, To_n, AnnualTave):
        ...
        f = (self.boundary_type == "external_general_part") \
            | (self.boundary_type == "external_transparent_part") \
        ...
        # 地面反射率[-]
        self.RhoG_l[f] = a18.get_RhoG()

        # 拡散日射の入射角特性
        self.Cd_i_k = a10.get_Cd(self.IAC_i_k)

        # 室外側表面総合熱伝達率 [W/m2K] 式(121)
        self.ho_i_k_n = a23.get_ho_i_k_n(self.Ro_i_k_n)

        # 室内側表面総合熱伝達率 [W/m2K] 式(122)
        self.hi_i_k_n = a23.get_hi_i_k_n(self.Ri_i_k_n)

        # 開口部の室内表面から屋外までの熱貫流率[W / (m2･K)] 式(124)
        f = np.logical_not((self.boundary_type == "external_general_part")
                           | (self.boundary_type == "internal")
                           | (self.boundary_type == "ground"))

        self.Uso[f] = a25.get_Uso(self.U[f], self.Ri_i_k_n[f])
        ...
        for k in range(self.N_surf_i):
            if self.boundary_type[k] in ["external_general_part", "internal", "ground"]:
                ...
                self.RFT0[k], self.RFA0[k], self.RFT1[k], self.RFA1[k], self.Row[k], self.Nroot[k] = \
                    a2.calc_response_factor(is_ground, self.C_i_k_p[k], self.R_i_k_p[k])
        ...
        # 1-1) 一般部位、不透明な開口部の場合
        f = tuple(f_sun &
                  ((self.boundary_type == "external_general_part") | (self.boundary_type == "external_opaque_part")))
        ...
        self.Teolist[f] = np.full(24 * 365 * 4, AnnualTave)

        # 2-2) 日の当たらない一般部位と内壁
        f = tuple(
            (f_sun == False) & ((self.boundary_type == "external_general_part") | (self.boundary_type == "internal")))
        self.Teolist[f] = np.zeros(24 * 365 * 4)
