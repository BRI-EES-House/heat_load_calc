...
class TestWindow(unittest.TestCase):
    ...
    def test_u_w_g_s_j(self):
        ...
        self.assertAlmostEqual(Window(u_w_std_j=3.0, eta_w_std_j=0.5, t_glz_j=GlassType.MULTIPLE, r_a_w_g_j=0.72, t_flame=FlameType.RESIN).glazing._u_w_g_s_j, 2.89843795)

    def test_r_r_w_g_j(self):
        # ((1 / 2) * (1 / 3.3111111 - 0.0415 - 0.1228) + 0.0756) * 2.89843795
        self.assertAlmostEqual(Window(u_w_std_j=3.0, eta_w_std_j=0.5, t_glz_j=GlassType.SINGLE, r_a_w_g_j=0.72, t_flame=FlameType.RESIN).glazing._r_r_w_g_j, 0.4186988)
        # ((1 / 4) * (1 / 3.3111111 - 0.0415 - 0.1228 - 0.003) + 0.0756) * 2.89843795
        self.assertAlmostEqual(Window(u_w_std_j=3.0, eta_w_std_j=0.5, t_glz_j=GlassType.MULTIPLE, r_a_w_g_j=0.72, t_flame=FlameType.RESIN).glazing._r_r_w_g_j, 0.3167365)
    ...
    def test_rho_w_g_s1f_j(self):
        ...
        self.assertAlmostEqual(Window(u_w_std_j=3.0, eta_w_std_j=0.5, t_glz_j=GlassType.SINGLE, r_a_w_g_j=0.72, t_flame=FlameType.RESIN).glazing._rho_w_g_s1f_j, 0.1402048)

        # t_j = (-1.846 * 0.3167365 + ((1.846 * 0.3167365)**2 + 4 * (1 - 1.846 * 0.3167365) * 0.6944444)**0.5) / (2 * (1 - 1.846 * 0.3167365))
        # 0.923 * (t_j ** 2) - 1.846 * t_j + 1
        self.assertAlmostEqual(Window(u_w_std_j=3.0, eta_w_std_j=0.5, t_glz_j=GlassType.MULTIPLE, r_a_w_g_j=0.72, t_flame=FlameType.RESIN).glazing._rho_w_g_s1f_j, 0.12652496)

    def test_rho_w_g_s2f_j(self):
        self.assertEqual(Window(u_w_std_j=3.0, eta_w_std_j=0.5, t_glz_j=GlassType.SINGLE, r_a_w_g_j=0.72, t_flame=FlameType.RESIN).glazing._rho_w_g_s2f_j, None)
        self.assertEqual(Window(u_w_std_j=3.0, eta_w_std_j=0.5, t_glz_j=GlassType.MULTIPLE, r_a_w_g_j=0.72, t_flame=FlameType.RESIN).glazing._rho_w_g_s2f_j, 0.077)
    ...
    def test_tau_w_g_s1_j(self):
        # 0.57534585
        self.assertAlmostEqual(Window(u_w_std_j=3.0, eta_w_std_j=0.5, t_glz_j=GlassType.SINGLE, r_a_w_g_j=0.72, t_flame=FlameType.RESIN).glazing._tau_w_g_s1_j, 0.57534585)
        # (0.379 * 0.077 * 0.63408559 + ((0.379 * 0.077 * 0.63408559)**2 - 4 * (0.379 * 0.077 - 1) * 0.63408559)**0.5) / 2
        self.assertAlmostEqual(Window(u_w_std_j=3.0, eta_w_std_j=0.5, t_glz_j=GlassType.MULTIPLE, r_a_w_g_j=0.72, t_flame=FlameType.RESIN).glazing._tau_w_g_s1_j, 0.79389655)

    def test_tau_w_g_s2_j(self):
        # 定義なし
        self.assertEqual(Window(u_w_std_j=3.0, eta_w_std_j=0.5, t_glz_j=GlassType.SINGLE, r_a_w_g_j=0.72, t_flame=FlameType.RESIN).glazing._tau_w_g_s2_j, None)
        # 0.79389655
        self.assertAlmostEqual(Window(u_w_std_j=3.0, eta_w_std_j=0.5, t_glz_j=GlassType.MULTIPLE, r_a_w_g_j=0.72, t_flame=FlameType.RESIN).glazing._tau_w_g_s2_j, 0.79389655)

    def test_rho_w_g_s1b_j(self):
        # 定義なし
        self.assertEqual(Window(u_w_std_j=3.0, eta_w_std_j=0.5, t_glz_j=GlassType.SINGLE, r_a_w_g_j=0.72, t_flame=FlameType.RESIN).glazing._rho_w_g_s1b_j, None)
        # 0.379 * (1 - 0.79389655)
        self.assertAlmostEqual(Window(u_w_std_j=3.0, eta_w_std_j=0.5, t_glz_j=GlassType.MULTIPLE, r_a_w_g_j=0.72, t_flame=FlameType.RESIN).glazing._rho_w_g_s1b_j, 0.07811320)
    ...
    def test_get_tau_w_g_j_phi_function(self):
        ...
        w_s = Window(u_w_std_j=3.0, eta_w_std_j=0.5, t_glz_j=GlassType.SINGLE, r_a_w_g_j=0.72, t_flame=FlameType.RESIN)
        # 0.57477050
        self.assertAlmostEqual(w_s.glazing._get_tau_w_g_j_phis(phis=radians(0.0)), 0.57477050)
        # 0.56908469
        self.assertAlmostEqual(w_s.glazing._get_tau_w_g_j_phis(phis=radians(30.0)), 0.56908469)
        # 0.50846189
        self.assertAlmostEqual(w_s.glazing._get_tau_w_g_j_phis(phis=radians(60.0)), 0.50846189)
        # 0.0
        self.assertAlmostEqual(w_s.glazing._get_tau_w_g_j_phis(phis=radians(90.0)), 0.0)

        w_m = Window(u_w_std_j=3.0, eta_w_std_j=0.5, t_glz_j=GlassType.MULTIPLE, r_a_w_g_j=0.72, t_flame=FlameType.RESIN)
        # 0.79310266 * 0.79310266 / (1 - 0.0781132 * 0.077)
        self.assertAlmostEqual(w_m.glazing._get_tau_w_g_j_phis(phis=radians(0.0)), 0.63281805)
        # 0.78525703 * 0.78525703 / (1 - 0.08080783 * 0.07969788)
        self.assertAlmostEqual(w_m.glazing._get_tau_w_g_j_phis(phis=radians(30.0)), 0.62062557)
        # 0.70160608 * 0.70160608 / (1 - 0.13518375 * 0.13413946)
        self.assertAlmostEqual(w_m.glazing._get_tau_w_g_j_phis(phis=radians(60.0)), 0.50134217)
        # 0.0 * 0.0 / (1 - 1.0 * 1.0)
        self.assertAlmostEqual(w_m.glazing._get_tau_w_g_j_phis(phis=radians(90.0)), 0.0)
    ...
    def test_get_b_w_g_j_phi_function(self):
        ...
        self.assertAlmostEqual(w_s.glazing._get_b_w_g_j_phis(phis=radians(0.0)), 0.11933949)
        # (1 - 0.56908469 - 0.14271797) * 0.41869881
        self.assertAlmostEqual(w_s.glazing._get_b_w_g_j_phis(phis=radians(30.0)), 0.12066788)
        # (1 - 0.50846189 - 0.19343152) * 0.41869881
        self.assertAlmostEqual(w_s.glazing._get_b_w_g_j_phis(phis=radians(60.0)), 0.12481687)
        # (1 - 0.0 - 0.99999999) * 0.41869881
        self.assertAlmostEqual(w_s.glazing._get_b_w_g_j_phis(phis=radians(90.0)), 0.0)
        w_m = Window(u_w_std_j=3.0, eta_w_std_j=0.5, t_glz_j=GlassType.MULTIPLE, r_a_w_g_j=0.72, t_flame=FlameType.RESIN)
        # 0.31673653
        self.assertAlmostEqual(w_m.glazing._r_r_w_g_j, 0.31673653)
        # (1 - 0.63281805 - 0.17525195) * 0.31673653
        self.assertAlmostEqual(w_m.glazing._get_b_w_g_j_phis(phis=radians(0.0)), 0.06079124)
        # (1 - 0.62062557 - 0.17854063) * 0.31673653
        self.assertAlmostEqual(w_m.glazing._get_b_w_g_j_phis(phis=radians(30.0)), 0.06361140)
        # (1 - 0.50134217 - 0.24784829) * 0.31673653
        self.assertAlmostEqual(w_m.glazing._get_b_w_g_j_phis(phis=radians(60.0)), 0.07944054)
        # (1 - 0.0 - 0.99999999) * 0.31673653
        self.assertAlmostEqual(w_m.glazing._get_b_w_g_j_phis(phis=radians(90.0)), 0.0)
