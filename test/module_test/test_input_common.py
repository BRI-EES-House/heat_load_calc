...
from heat_load_calc import input_common
...
def test_common_interval1():
    ...
    ipt_common = input_common.InputCommon.read(d_common=d_common)
...
def test_common_weather7():
    ...
    ipt_weather: input_common.InputWeatherEES = ipt_common.ipt_weather
...
def test_common_weather13():
    ...
    ipt_weather: input_common.InputWeatherFile = ipt_common.ipt_weather
...
def test_common_season6():
    ...
    with pytest.raises(ValueError) as e:
        input_common.InputCommon.read(d_common=f('7/1/1', '9/30'))
    
    assert 'Value of summer_start is wrong format or not exist day.' in str(e.value)

    with pytest.raises(ValueError) as e:
        input_common.InputCommon.read(d_common=f('7/32', '9/30'))
    
    assert 'Value of summer_start is wrong format or not exist day.' in str(e.value)

    with pytest.raises(TypeError) as e:
        input_common.InputCommon.read(d_common=f(2, '9/30'))
    
    assert 'Value of summer_start is not str.' in str(e.value)

    with pytest.raises(ValueError) as e:
        input_common.InputCommon.read(d_common=f('7/1', '9/30/1'))
    
    assert 'Value of summer_end is wrong format or not exist day.' in str(e.value)

    with pytest.raises(ValueError) as e:
        input_common.InputCommon.read(d_common=f('7/1', '9/31'))
    
    assert 'Value of summer_end is wrong format or not exist day.' in str(e.value)

    with pytest.raises(TypeError) as e:
        input_common.InputCommon.read(d_common=f('7/1', 2))
...
def test_common_season7():
    ...
    with pytest.raises(ValueError) as e:
        input_common.InputCommon.read(d_common=f('11/1/1', '3/31'))
    
    assert 'Value of winter_start is wrong format or not exist day.' in str(e.value)

    with pytest.raises(ValueError) as e:
        input_common.InputCommon.read(d_common=f('11/32', '3/31'))
    
    assert 'Value of winter_start is wrong format or not exist day.' in str(e.value)

    with pytest.raises(TypeError) as e:
        input_common.InputCommon.read(d_common=f(2, '3/31'))
    
    assert 'Value of winter_start is not str.' in str(e.value)

    with pytest.raises(ValueError) as e:
        input_common.InputCommon.read(d_common=f('11/1', '3/31/1'))
    
    assert 'Value of winter_end is wrong format or not exist day.' in str(e.value)

    with pytest.raises(ValueError) as e:
        input_common.InputCommon.read(d_common=f('11/1', '3/32'))
    
    assert 'Value of winter_end is wrong format or not exist day.' in str(e.value)

    with pytest.raises(TypeError) as e:
        input_common.InputCommon.read(d_common=f('11/1', 2))
...
def test_common_number_of_occupants2():
    ...
    ipt_common = input_common.InputCommon.read(d_common=f('1'))

    assert ipt_common.n_ocp == ENumberOfOccupants.One

    ipt_common = input_common.InputCommon.read(d_common=f('2'))

    assert ipt_common.n_ocp == ENumberOfOccupants.Two

    ipt_common = input_common.InputCommon.read(d_common=f('3'))

    assert ipt_common.n_ocp == ENumberOfOccupants.Three

    ipt_common = input_common.InputCommon.read(d_common=f('4'))

    assert ipt_common.n_ocp == ENumberOfOccupants.Four

    with pytest.raises(ValueError) as e:
        input_common.InputCommon.read(d_common=f('5'))
