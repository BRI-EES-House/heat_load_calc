...
加湿・除湿を行わない場合のステップ |n+1| における室 |i| の絶対湿度 :math:`X_{ntr,i,n+1}` は式(6)で表される。
...
ステップ |n| における係数 :math:`F_{t,wgt,i,j,n}` は式(7)で表される。
...
ステップ |n| における室の湿度に関する係数 :math:`F_{t,cst,i,n}` は式(8)で表される。
...
ステップ |n+1| における室 |i| に設置されたルームエアコンディショナーの室内機の熱交換器表面絶対湿度 :math:`X_{rac,ex-srf,i,n+1}` は式(12)で表される。
...
ステップ |n+1| における室 |i| に設置されたルームエアコンディショナーの室内機の熱交換器表面温度　:math:`\theta_{rac,ex-srf,i,n+1}` は式(13)で表される。
...
ステップ |n| から |n+1| における室 |i| に設置されたルームエアコンディショナーの吹き出し風量 :math:`\hat{V}_{rac,i,n}` は式(14)により表される。
...
における、室 |i| の要素 :math:`X'_{r,i,n+1}` または :math:`\hat{L}'_{b,set,i,n}` について、どちらかは必ずゼロになるため、前述の :math:`\hat{f}_{set,i,n}` （添字はiとした）を用いて、
...
ここで、機器の室内機の吹き出し風量を :math:`V_{eq}` とすると、除湿量は、
