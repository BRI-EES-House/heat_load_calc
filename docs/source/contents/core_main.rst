...
:math:`\hat{V}_{vent,int,i,i*,n}`
    | ステップ |n| からステップ |n+1| における室 |i*| から室 |i| への室間の空気移動量, |m3|/s
...
:math:`\theta_{frt,i,n}`
    | ステップ |n| における室 |i| に設置された備品等の温度, ℃
:math:`\theta_{lower,target,i,n}`
    | ステップ |n| における室 |i| の作用温度下限値 , ℃
...
係数 :math:`\pmb{F}_{WSV,n+1}` は、式(27)により表される。
