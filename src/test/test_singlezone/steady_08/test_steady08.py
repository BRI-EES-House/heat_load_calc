...
class TestSteadyState(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        c_n = conditions.Conditions(
            ...
            theta_dsh_s_t_js_ms_n=(np.dot(ss.bs.k_ei_js_js, theta_ei_js_n) + ss.bs.k_eo_js * ss.bs.theta_o_eqv_js_ns[:, 1].reshape(-1, 1) + np.dot(ss.bs.k_s_r_js_is, theta_r_is_n)) * ss.bs.phi_t1_js_ms / (1.0 - ss.bs.r_js_ms),
            q_s_js_n=q_srf_js_n,
        ...
        # 計算実行
        c_n_pls = pre_calc_parameters.run_tick(n=-2, delta_t=900.0, ss=ss, c_n=c_n, recorder=None)
