...
def run_tick(n: int, delta_t: float, ss: PreCalcParameters, c_n: Conditions, recorder: Recorder) -> Conditions:
    ...
    def comp(name, x, y, view_detail=False):
        if not view_detail:
            result = np.all(np.abs(x - y) < 0.00000001)
    ...
    if n == 21071:
        ...
        #     + np.sum(theta_dsh_s_t_js_ms_n_pls, axis=1).reshape(-1, 1)
        # )
        # print('theta_s_js_n_pls')
        # print(theta_s_js_n_pls)
        # print('計算値')
        # print(ss.phi_a0_js * q_s_js_n_pls
        #       + np.sum(theta_dsh_s_a_js_ms_n_pls, axis=1).reshape(-1, 1)
        #       + ss.phi_t0_js * theta_rear_js_n_pls
        #       + np.sum(theta_dsh_s_t_js_ms_n_pls, axis=1).reshape(-1, 1)
        # )
        comp(
            name='式(8)の検証（ただしnはn+1と読み替える）',
            x=q_s_js_n_pls,
            y=(ss.h_s_c_js+ ss.h_s_r_js) * (theta_ei_js_n_pls - theta_s_js_n_pls)
        )
        comp(
            name='式(13)の検証',
            x=theta_rear_js_n_pls,
            y=np.dot(ss.k_ei_js_js, theta_ei_js_n_pls)
                + np.dot(ss.k_s_r_js_is, theta_r_is_n_pls)
                + ss.k_eo_js * ss.theta_o_eqv_js_ns[:, n+1].reshape(-1, 1)
        )
        comp(
        ...
        comp(
            name='式(16-2)の検証',
            x=theta_s_js_n_pls,
            y=ss.phi_a0_js * (ss.h_s_r_js + ss.h_s_c_js) * (theta_ei_js_n_pls - theta_s_js_n_pls)
                + ss.phi_t0_js * (np.dot(ss.k_ei_js_js, theta_ei_js_n_pls) + np.dot(ss.k_s_r_js_is, theta_r_is_n_pls) + ss.k_eo_js * ss.theta_o_eqv_js_ns[:, n+1].reshape(-1, 1))
                + np.sum(theta_dsh_s_a_js_ms_n_pls, axis=1).reshape(-1, 1)
                + np.sum(theta_dsh_s_t_js_ms_n_pls, axis=1).reshape(-1, 1)
        )
        comp(
            name='式(16-3)の検証',
            x=theta_s_js_n_pls,
            y=ss.phi_a0_js * (ss.h_s_r_js + ss.h_s_c_js) / (ss.h_s_r_js + ss.h_s_c_js) * (
                    ss.h_s_c_js * np.dot(ss.p_js_is, theta_r_is_n_pls)
                    + ss.h_s_r_js * np.dot(ss.p_js_is, np.dot(ss.f_mrt_is_js, theta_s_js_n_pls))
                    + ss.q_s_sol_js_ns[:, n+1].reshape(-1, 1)
                    + np.dot(f_flr_js_is_n, (1 -beta_is_n) * l_rs_is_n) / ss.a_s_js
                ...
                + ss.phi_t0_js * np.dot(ss.k_ei_js_js, (
                    ss.h_s_c_js * np.dot(ss.p_js_is, theta_r_is_n_pls)
                    + ss.h_s_r_js * np.dot(ss.p_js_is, np.dot(ss.f_mrt_is_js, theta_s_js_n_pls))
                    + ss.q_s_sol_js_ns[:, n+1].reshape(-1, 1)
                    + np.dot(f_flr_js_is_n, (1 -beta_is_n) * l_rs_is_n) / ss.a_s_js
        ...
        comp(
            ...
            y=ss.phi_a0_js * ss.h_s_c_js * np.dot(ss.p_js_is, theta_r_is_n_pls)
                + ss.phi_a0_js * ss.h_s_r_js * np.dot(ss.p_js_is, np.dot(ss.f_mrt_is_js, theta_s_js_n_pls))
                + ss.phi_a0_js * ss.q_s_sol_js_ns[:, n+1].reshape(-1, 1)
                + ss.phi_a0_js / ss.a_s_js * np.dot(f_flr_js_is_n, (1 - beta_is_n) * l_rs_is_n)
                - ss.phi_a0_js * (ss.h_s_r_js + ss.h_s_c_js) * theta_s_js_n_pls
                + ss.phi_t0_js * np.dot(ss.k_ei_js_js, ss.h_s_c_js / (ss.h_s_c_js + ss.h_s_r_js) * np.dot(ss.p_js_is, theta_r_is_n_pls))
                ...
                + ss.phi_t0_js * np.dot(ss.k_ei_js_js, ss.q_s_sol_js_ns[:, n+1].reshape(-1, 1) / (ss.h_s_c_js + ss.h_s_r_js))
                + ss.phi_t0_js * np.dot(ss.k_ei_js_js, np.dot(f_flr_js_is_n, (1 - beta_is_n) * l_rs_is_n) / (ss.h_s_c_js + ss.h_s_r_js) / ss.a_s_js)
                + ss.phi_t0_js * np.dot(ss.k_s_r_js_is, theta_r_is_n_pls)
                + ss.phi_t0_js * ss.k_eo_js * ss.theta_o_eqv_js_ns[:, n+1].reshape(-1, 1)
                + np.sum(theta_dsh_s_a_js_ms_n_pls, axis=1).reshape(-1, 1)
                + np.sum(theta_dsh_s_t_js_ms_n_pls, axis=1).reshape(-1, 1)
        ...
        comp(
            ...
            y=np.diag((1 + ss.phi_a0_js * (ss.h_s_c_js + ss.h_s_r_js)).flatten())
                ...
                - ss.phi_t0_js * np.dot(ss.k_ei_js_js, np.dot(ss.p_js_is, ss.f_mrt_is_js) / (ss.h_s_r_js + ss.h_s_c_js) * ss.h_s_r_js)
        )
        comp(
            name='式(19)の検証',
