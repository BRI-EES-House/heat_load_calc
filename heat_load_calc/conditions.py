...
class Conditions:
    ...
    def initialize_conditions(cls, n_r: int, n_b: int, is_ground: np.array, gc_n: GroundConditions):
        ...
        # ステップnにおける室iの運転状態, [i, 1]
        # 初期値を暖房・冷房停止で窓「閉」とする。
        operation_mode_is_n = np.full((n_r, 1), OperationMode.STOP_CLOSE)

        # ステップnにおける室iの空気温度, degree C, [i, 1]
        # 初期値を15℃とする。
        theta_r_is_n = np.full((n_r, 1), 15.0)

        # ステップnにおける室iの在室者の平均放射温度, degree C, [i, 1]
        # 初期値を15℃と設定する。
        theta_mrt_hum_is_n = np.full((n_r, 1), 15.0)

        # ステップnにおける室iの絶対湿度, kg/kgDA, [i, 1]
        # 初期値を空気温度20℃相対湿度40%の時の値とする。
        # x_r_is_n = np.full((total_number_of_spaces, 1), psy.get_x(psy.get_p_vs(theta=20.0) * 0.4))
        x_r_is_n = np.full((n_r, 1), psy.get_x(psy.get_p_vs(theta=20.0) * 0.4))

        # ステップnの境界jにおける表面熱流（壁体吸熱を正とする）, W/m2, [j, 1]
        # 初期値を0.0W/m2とする。
        q_srf_jstrs_n = np.zeros((n_b, 1), dtype=float)

        # ステップnの室iにおける家具の温度, degree C, [i]
        # 初期値を15℃とする。
        theta_frt_is_n = np.full(n_r, 15.0)

        # ステップnの室iにおける家具の絶対湿度, kg/kgDA, [i, 1]
        # 初期値を空気温度20℃相対湿度40%の時の値とする。
        x_frt_is_n = np.full((n_r, 1), psy.get_x(psy.get_p_vs(theta=20.0) * 0.4))
